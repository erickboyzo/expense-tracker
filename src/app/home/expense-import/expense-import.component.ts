        ...
        if (results.data.length > 0) {
          if (
            requiredColumns.every(
              (x) => results.meta.fields.includes(x) || results.meta.fields.includes(x.toLowerCase()),
            )
          ) {
            this.dataExported.emit(results.data);
          } else {
            this.snackBar.open(
              `Error when processing csv file. Missing columns ${requiredColumns.filter((x) => !results.meta.fields.includes(x))}`,
              undefined,
              { duration: 3000 },
            );
          ...
          this.snackBar.open('No data found! Please check csv file and try again.', undefined, { duration: 3000 });
        }
      },
      header: true,
      skipEmptyLines: true,
